...
5. **Custom Hook Layer (`src/hooks/`)**: Encapsulated stateful logic (cryptographic ledger chain, CLI terminal simulation, scroll tracking, intersection reveals, theme toggling).
...
│  - NavBar                    │  - Projects (with Diagrams)  │
│  - Hero (with Network Canvas)│  - Publications (5 IEEE)     │
│  - Stats                     │  - Education                 │
│  - About (with MERN flow)    │  - Certifications            │
│  - Experience (Hash Chain)   │  - Resume Section            │
│  - Skills                    │  - Contact & Terminal        │
│                              │  - Footer                    │
├──────────────────────────────┴──────────────────────────────┤
│                       Shared Layers                         │
...
### `src/app/`
- **`App.js`**: Main layout orchestrator. Manages modal dialog state, renders top-of-page navigation, mounts critical above-the-fold features synchronously, and loads below-the-fold features asynchronously via `React.lazy()` + `<Suspense fallback={<SectionFallback />} />`.
- **`App.module.css`**: Styling for lazy-loading skeleton fallbacks and spinner animations.
- **`App.test.js`**: Integration test verifying synchronous mount of hero and asynchronous resolution of lazy chunks.
...
### `src/features/`
- **`home/Hero.js`**: Hero landing section. Renders engineering identity badges, Genesis Block metadata (`0x000000000000`), bio tagline, CTA actions, and mounts the interactive `EngineeringNetwork` visual.
- **`home/Stats.js`**: Metric cards (4+ Years, 5 IEEE Publications, 18+ Vulns Detected, MeitY Service) animated via `useReveal`.
...
- **`experience/Experience.js`**: Reverse-chronological cryptographic career ledger. Uses `useHashChain` to compute a linked chain of block hashes representing positions at Rashtriya Raksha University and C-DAC (MeitY).
- **`skills/Skills.js`**: Interactive skills matrix categorized into Blockchain & Web3, Cybersecurity & Auditing, and Full Stack MERN, with instant filter switching.
- **`projects/Projects.js`**: Featured project cards for **NBFDBAudit**, **BSAP Portal**, **National Blockchain Framework**, and **E-Waste DApp**. Includes interactive expandable architecture flow diagrams.
- **`publications/Publications.js`**: All 5 peer-reviewed IEEE publications with verified dates, conference venues, document IDs, and direct links to IEEE Xplore.
- **`education/Education.js`**: Verified degree credentials from GTU (M.E. Cybersecurity) and MEF (B.E. Computer Engineering).
- **`certifications/Certifications.js`**: Security accreditations (CBSP Blockchain Council, EC-Council DevSecOps, C-DAC Performer of the Month, NIELIT).
- **`contact/Contact.js`**: Dual-column communication hub embedding the controlled `ContactForm`, simulated REST JSON API response card, and the interactive CLI `Terminal`.
- **`contact/ContactForm.js`**: Controlled input form using `async/await` and `submitContactForm` service with explicit validation, loading, success, and error alert states.
- **`contact/Terminal.js`**: Interactive shell terminal with command execution, history recall, status dots, and async `ping` network simulation.
- **`resume/ResumeSection.js`**: Dedicated banner providing direct download of `Neelkumar_Shah_CV.pdf` and triggering the in-browser `CvModal`.
...
- **`animations/MernArchitecture/MernArchitecture.js`**: Interactive 4-tier MERN stack diagram with interactive inspection pane detailing client, gateway, service, and database contributions.
...
- **`layout/`**: Global shell components (`NavBar`, `Footer`, `ScrollProgressBar`) with active section tracking, theme switcher, and rAF-throttled scroll progress.
...
- **`useScrollProgress.js`**: High-performance rAF-throttled scroll listener that mutates DOM width directly to prevent unnecessary React re-renders.
...
- **Hero Display**: `Space Grotesk` (700 weight, `3.5rem` mobile / `4.5rem` desktop)
- **Section Headings (H2)**: `Space Grotesk` (600 weight, `2.25rem`)
- **Card Headings (H3/H4)**: `Space Grotesk` (600/700 weight, `1.25rem` – `1.75rem`)
- **Navigation & Links**: `Inter` (500 weight, `0.875rem`)
- **Body Copy**: `Inter` (400 weight, `1rem`)
...
- **Dark Palette**: Canvas `#090d16`, Surface-2 `#141c2e`, Border `#22304d`, Brand Blue `#3b82f6`, Security Green `#10b981`, MERN Cyan `#06b6d4`, Text `#f1f5f9`.
- **Light Palette**: Canvas `#f8fafc`, Surface-2 `#ffffff`, Border `#cbd5e1`, Brand Blue `#2563eb`, Security Green `#059669`, MERN Cyan `#0891b2`, Text `#0f172a`.
...
### D. rAF-Throttled Scroll Progress Tracking (`src/hooks/useScrollProgress.js`)
- High-frequency `window.onscroll` events do NOT call `setState()` (which would trigger full React virtual DOM diffing on every pixel scrolled).
- Instead, the handler sets a boolean flag (`ticking = true`) and schedules a single `requestAnimationFrame` callback.
- The callback updates the DOM node's `style.width` directly in $O(1)$ time, yielding silky 60/120 FPS performance.
...
3. **Backend API Readiness**: The contact service (`contactService.js`) is designed as a drop-in adapter. Connecting a live AWS Lambda, Express, or Next.js backend requires updating only the endpoint URL in `siteConfig.js`.
...
- **Reduced Motion**: Full support for `@media (prefers-reduced-motion: reduce)`. Animations are disabled or clamped to 0.01ms, and canvas rendering switches to single-frame static mode.
